 ...
 */
@Controller
public class AppBannerAct {

    @RequestMapping(value = "/app/banner", method = RequestMethod.POST)
    public void bannersave(String[] urls, String[] imgs, Integer cate, HttpServletResponse response) {
    ...
    @RequestMapping(value = "/app/banner/list", method = RequestMethod.POST)
    public void getBanners(Integer cate, HttpServletResponse response) {
        JsonObject obj = new JsonObject();
        if (null == cate) {
            obj.addProperty("status", 0);
            obj.addProperty("msg", "cate not defined ...");
